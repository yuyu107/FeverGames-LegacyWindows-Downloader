...
正式 v1.3.5 Release ZIP 直接包含已经测试过的 x64 `libzstd.dll`。
...
v1.3.5 的常用 Release CMD 入口已经加入检测提示；仍建议始终先完整解压 ZIP。
...
v1.3.5 Release ZIP 不再依赖系统 7-Zip 安装路径。
...
因此这些并发参数不进入 v1.3.5 正式版。正式版保持更保守、已经验证过的串行下载 / 重组路径。
